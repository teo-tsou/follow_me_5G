...
class MecEnv(gym.Env):
    ...
    def prepare_target_migration(self,target_node):
        host_ip = self.nodes_to_ips[target_node]
        ...
        try:
            ...
            for key in self.nodes:
                if key != str(host_ip):  
                    try:
                        #Un-label:
                        result = subprocess.check_call(['kubectl', 'label', 'nodes', self.nodes[key]["name"], "migration-" ])
                    except:
                        pass    
                else:
                    try:
                    ...
                    try:
                        #Un-label:
                        result = subprocess.check_call(['kubectl', 'label', 'nodes', self.nodes[key]["name"], "migration=true" ])
                    except:
                        pass
        finally:
            self.lock.release()
    ...
    def vm_migrate(self,vm_name="", target_node = "", migration_file = "amf_migration.yaml"):  
        source_node = self.get_vm_host(vm_name)
        if source_node == 0:
        ...
        self.prepare_target_migration(target_node)    

        #Delete Previous Migration Jobs    
        try:
            result = subprocess.check_call(['kubectl', 'delete', '-f',f'{self.VM_MIGRATION_PATH}/{migration_file}'])
        except:
            pass
        #Create live migration job for the coressponding Core Network Function
        result = subprocess.check_output(['kubectl', 'create', '-f', f'{self.VM_MIGRATION_PATH}/{migration_file}'])
        if "created" in str(result):
            print("going for migration")
            start = time.time()
            active_vm_migration = 1
            target_node = self.get_vm_host(vm_name)
            while(source_node == target_node):
                target_node = self.get_vm_host(vm_name)
            end = time.time()
            active_vm_migration = 0
            migration_time = end-start
            node = self.get_vm_host(vm_name)
            host_ip = self.nodes_to_ips[node]
            for key in self.nodes:
                if key != str(host_ip):  
                    self.nodes[key]["vms"] = ""
                else:
                    self.nodes[host_ip]["vms"] = vm_name
    ...
    def mec_source_host(self,mec_name="", ns="default"):
    ...
    def pod_migrate(self, mec_name="", ns="default", target_node=""):
        host_ip,pod_name = self.mec_source_host(mec_name,ns)
        if host_ip == 0 or pod_name == 0:
            print("No mec-app running on any host")
            return 0
        if (host_ip == self.nodes_to_ips[target_node]):
            print('Migration Error: The source node is same us the target node')
            return 0
        ...
        if "response Status: 200 OK" in str(result):
            ...
            while(host_ip != self.nodes_to_ips[target_node]):
                host_ip,pod_name = self.mec_source_host(mec_name,ns)
            end = time.time()
            print("")
            ...
            for key in self.nodes:
                if key != str(host_ip):  
                    self.nodes[host_ip]["vms"] = ""
                else:
                    self.nodes[host_ip]["vms"] = pod_name
